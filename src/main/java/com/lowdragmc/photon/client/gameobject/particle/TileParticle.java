...
public class TileParticle implements IParticle {
    ...
    public void setup() {
        ...
        setDelay(config.getStartDelay().get(emitterT, randomSource::nextFloat).intValue());
        if (config.lifetimeByEmitterSpeed.isEnable()) {
            setLifetime(config.lifetimeByEmitterSpeed.getLifetime(this, emitter,
                    config.getStartLifetime().get(emitterT, randomSource::nextFloat).intValue()));
        } else {
            setLifetime(config.getStartLifetime().get(emitterT, randomSource::nextFloat).intValue());
        }
        ...
        mulInternalVelocity(config.getStartSpeed().get(emitterT, randomSource::nextFloat).floatValue());
        this.initialSize = config.getStartSize().get(emitterT, randomSource::nextFloat);
        this.initialRotation = config.getStartRotation().get(emitterT, randomSource::nextFloat).mul(Mth.TWO_PI / 360);
        var color = config.getStartColor().get(emitterT, randomSource::nextFloat).intValue();
