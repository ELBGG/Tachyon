...
@Setter
@Getter
public class EmissionSetting implements IPersistedSerializable {
    ...
    public int getEmissionCount(ParticleEmitter particleEmitter, RandomSource randomSource) {
        ...
        var timeValue = emissionRate.get(t, randomSource::nextFloat);
        var distanceValue = distanceRate.get(t, randomSource::nextFloat).floatValue();
        ...
        for (var bust : bursts) {
            ...
            if (realAge >= 0) {
                var count = bust.count.get(t, randomSource::nextFloat).intValue();
    ...
    public static class Burst implements IPersistedSerializable{
